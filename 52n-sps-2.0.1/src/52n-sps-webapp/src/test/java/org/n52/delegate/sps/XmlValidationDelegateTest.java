...
public class XmlValidationDelegateTest {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(XmlValidationDelegateTest.class);
	...
	private SPSXmlValidationDelegate validatingDelegate;

	@Before
	public void setUp() throws Exception {
		SensorPlanningService service = createSpsMockup();
		SPSXmlDelegate delagate = new SPSXmlDelegate(service, null);
		validatingDelegate = new SPSXmlValidationDelegate(delagate);
	}
	...
	private void assertMissingParameterValueException(String parameter)
			throws OwsException {
		try {
	...
	private void assertInvalidParamterValueException(String parameter)
			throws OwsException {
		try {
			validatingDelegate.validateServiceParameter(parameter);
	...
	@Test(expected = MissingParameterValueException.class)
	public void testMissingServiceInGetCapabilitiesRequest()
			throws XmlException, IOException, OwsException {
		XmlObject getCapabilities = getRequest(REQUEST_GET_CAPABILITIES_MISSING_SERVICE);
		validatingDelegate
				.assureCorrectServiceAndVersionRequestParameters(getCapabilities);
	}
