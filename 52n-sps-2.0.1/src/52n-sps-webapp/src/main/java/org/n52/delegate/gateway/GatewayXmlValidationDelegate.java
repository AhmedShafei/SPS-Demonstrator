...
public class GatewayXmlValidationDelegate implements DelegationHandler {

	private GatewayXmlDelegate requestHandler;
	private SensorPlanningService service;

	public GatewayXmlValidationDelegate(GatewayXmlDelegate toDecorate) {
	...
	public XmlObject delegate() throws OwsException, OwsExceptionReport,
			XmlException {
		assureCorrectServiceAndVersionRequestParameters(requestHandler
				.getRequest());
		validateSwesRequest(requestHandler.getRequest());
		return requestHandler.delegate();
	}

	private void validateSwesRequest(XmlObject swesRequest)
	...
	protected void assureCorrectServiceAndVersionRequestParameters(
			XmlObject request) throws OwsException {
	...
	protected void validateRequiredServiceParameter(XmlObject requestDocument)
			throws OwsException {
	...
	protected void validateRequiredVersionParameter(XmlObject requestDocument)
			throws OwsException {
		XmlCursor xmlCursor = requestDocument.newCursor();
		xmlCursor.toFirstChild();
	...
	private boolean moveToAttribute(final XmlCursor xmlCursor,
	...
	private boolean isOnAttribute(final String attribute,
			final XmlCursor xmlCursor) {
		return xmlCursor.isAttr()
				&& xmlCursor.getName().equals(new QName(attribute));
	}

	protected void validateServiceParameter(String serviceParameter)
			throws OwsException {
