...
import org.n52.sps.sensor.wos.exec.WosTaskScheduler;
...
public class WosTestSensor extends SensorPlugin {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(WosTestSensor.class);

	private WosTaskScheduler scheduler;

	public WosTestSensor(SensorTaskService sensorTaskService,
			SensorConfiguration configuration) throws InternalServiceException {
		super(sensorTaskService, configuration);
		scheduler = new WosTaskScheduler(sensorTaskService);
	}
	...
	public Availability getResultAccessibilityFor(SensorTask sensorTask) {
		WosTaskResultAccess resultAccess = createResultAccessGenerator(sensorTask);
		return resultAccess.getResultAccessibility();
	}

	private WosTaskResultAccess createResultAccessGenerator(
			SensorTask sensorTask) {
		return new WosTaskResultAccess(sensorTask,
