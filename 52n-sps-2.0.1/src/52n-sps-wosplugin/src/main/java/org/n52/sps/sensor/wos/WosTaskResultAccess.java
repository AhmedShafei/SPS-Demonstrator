...
public class WosTaskResultAccess implements ResultAccessAvailabilityDescriptor {

	private ResultAccessDataServiceReference resultAccessDataServiceReference;

	private SensorTask sensorTask;

	public WosTaskResultAccess(SensorTask sensorTask,
			ResultAccessDataServiceReference dataReference) {
	...
	private Availability createDataNotAvailable(String message) {
		LanguageStringType languageMsg = LanguageStringType.Factory
				.newInstance();
	...
	private Availability createDataAvailibility() {
		...
		ReferenceGroupType referenceGroup = dataReference
				.addNewReferenceGroup();
		referenceGroup.addNewIdentifier()
				.setStringValue(sensorTask.getTaskId());

		referenceGroup.addNewAbstractReferenceBase().set(
		...
		return new DataAvailable(new DataReference[] { dataReference })
				.getResultAccessibility();
	}

	/**
	 * The {@link WosTaskResultAccess} applies the service reference template
	 * provided when sensor instance gets installed. SPS 2.0.0 specification
	 * allows five options to create a result access. This method creates a
	 * reference regarding to Option 2 (see SPS 2.0.0 spec., table 41) with role
	 * 
	 * <pre>
	...
	ServiceReferenceType createServiceReference() {
		ServiceReferenceType serviceReference = ServiceReferenceType.Factory
				.newInstance();
		serviceReference.addNewIdentifier().setStringValue(
				sensorTask.getTaskId());
		serviceReference.setHref(resultAccessDataServiceReference
				.getReference());
		serviceReference.setRole(resultAccessDataServiceReference.getRole());
		serviceReference.setTitle(String.format("Observation data for task %s",
				sensorTask.getTaskId()));
		serviceReference
				.setFormat(resultAccessDataServiceReference.getFormat());
		...
		accessMetadata
				.setDataAccessType("http://www.opengis.net/sos/2.0/GetObservation");
		serviceReference.addNewMetadata().addNewAbstractMetaData()
				.set(accessMetadata);

		for (MetadataType metadata : serviceReference.getMetadataArray()) {
			// XXX check if there is a better way to qualify abstract element
			QName qname = new QName("http://www.opengis.net/sps/2.0",
					"SPSMetadata");
			XMLBeansTools.qualifySubstitutionGroup(
					metadata.getAbstractMetaData(), qname);
		}
	...
	String createRequest() {
		...
		sb.append("<GetObservationById ").append("version='2.0.0' ")
				.append("service='SOS' ");
