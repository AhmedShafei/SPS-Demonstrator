...
public class WosTestSensorTest extends TestCase {
	...
	private static final String PLUGIN_TYPE = "org.n52.sps.sensor.wos.WosTestSensor";

	private WosTestSensor sensor;

	@Override
	protected void setUp() throws Exception {
		SensorConfiguration configuration = new SensorConfiguration();
		configuration.setProcedure(PROCEDURE);
		XmlObject template = FileContentLoader.loadXmlFileViaClassloader(
				TASKING_PARAMETERS_TEMPLATE, getClass());
		configuration
				.setTaskingParametersTemplate(((DataRecordDocument) template)
						.getAbstractDataComponent());
		configuration.setSensorPluginType(PLUGIN_TYPE);
		configuration.addSensorDescription(new SensorDescription(
				PROCEDURE_DESCRIPTION_FORMAT, PROCEDURE_DESCRIPTION_URL));
		sensor = new WosTestSensor(new SensorTaskService(
				new InMemorySensorTaskRepository()), configuration);
	}
	...
	public void testCreateSpsMetadata() {
		...
		try {
			SPSMetadataDocument parsedmetadata = SPSMetadataDocument.Factory
					.parse(metadata.getDomNode());
			assertEquals("Types are not equal!", SPSMetadataDocument.type,
					parsedmetadata.schemaType());
	...
	public void testQualifyDataComponent() {
		AbstractDataComponentType taskingParametersTemplate = sensor
				.getSensorConfiguration().getTaskingParametersTemplate();
		...
		TaskingParameters taskingParameter = describeTaskingResponse
				.addNewTaskingParameters();
		taskingParameter.addNewAbstractDataComponent().set(
				taskingParametersTemplate);

		TaskingParameters taskingParameters = describeTaskingResponse
				.getTaskingParameters();
		AbstractDataComponentType unQualifiedDataComponent = taskingParameters
				.getAbstractDataComponent();
		...
		assertEquals("Types are not equal!", DataRecordType.type,
				qualifiedDataComponent.schemaType());
	}
