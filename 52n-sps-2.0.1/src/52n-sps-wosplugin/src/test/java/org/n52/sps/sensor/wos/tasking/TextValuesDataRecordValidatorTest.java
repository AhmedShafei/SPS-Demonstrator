...
import org.n52.sps.sensor.wos.tasking.TextValuesDataRecordValidator;
...
public class TextValuesDataRecordValidatorTest {
	...
	private DataRecordType getTaskingParamters() throws XmlException,
			IOException {
	...
	private SubmitTaskingRequest getValidTaskingRequest() throws XmlException,
			IOException {
		XmlObject template = FileContentLoader.loadXmlFileViaClassloader(
				VALID_SUBMIT_REQUEST, getClass());
	...
	public void testGetValidDataRecords() {
		try {
			// XXX test not complete ... FieldArray cannot be loaded during test
			// => solve
			validator = new TextValuesDataRecordValidator(dataRecordTemplate,
					validTaskingRequest);
			validator.getValidDataRecords();
		} catch (InvalidParameterValueException e) {
			fail("Could not create valid data records from tasking request: "
					+ validTaskingRequest);
			e.printStackTrace();
		}
	}
