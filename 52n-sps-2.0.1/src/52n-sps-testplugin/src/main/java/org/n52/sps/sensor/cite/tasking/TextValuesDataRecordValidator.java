...
public class TextValuesDataRecordValidator {
	...
	private void validateAndSet(Field field, String[] parameters,
		...
		try {
			...
			if (name.equalsIgnoreCase(ParameterField.MEASUREMENT_FREQUENCY
					.getFieldName())) {
				QuantityType quantityToSet = validator.validateQuantity(field,
						parameters[0]);
				validData.addNewField().setAbstractDataComponent(quantityToSet);
			} else if (name.equalsIgnoreCase(ParameterField.MEASUREMENT_COUNT
					.getFieldName())) {
				CountType countToSet = validator.validateCount(field,
						parameters[0]);
				validData.addNewField().setAbstractDataComponent(countToSet);
			} else if (name
					.equalsIgnoreCase(ParameterField.MEASUREMENT_LOCATION
							.getFieldName())) {
				VectorType value = create2DQuantityVector(parameters);
				VectorType coords = validator.validateQuantityVector(field,
						value);
				validData.addNewField().setAbstractDataComponent(coords);
			} else if (name
					.equalsIgnoreCase(ParameterField.MAX_MISSION_DURATION
							.getFieldName())) {
				// TODO add validation methods
			} else if (name.equalsIgnoreCase(ParameterField.MEASUREMENT_PURPOSE
					.getFieldName())) {
				// TODO add validation methods
			} else if (name.equalsIgnoreCase(ParameterField.SHALL_MEASURE
					.getFieldName())) {
				// TODO add validation methods
			} else if (name
					.equalsIgnoreCase(ParameterField.MEASUREMENT_PRIORITY
							.getFieldName())) {
				// TODO add validation methods
			}
