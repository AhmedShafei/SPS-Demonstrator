...
/**
 * Serves as test mockup and does not provide test code for testing implementing
 * methods. To be used to test {@link SensorPlugin} methods in general.
 */
public class SimpleSensorPluginTestInstance extends SensorPlugin {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(SimpleSensorPluginTestInstance.class);
	...
	/**
	 * Creates a plugin instance for testing. It creates its
	 * {@link InMemorySensorTaskRepository} instance.
	 * 
	 * @return a very simple instance of {@link SensorPlugin} for testing
	 *         purposes.
	...
	public static SensorPlugin createInstance() throws InternalServiceException {
		...
		return new SimpleSensorPluginTestInstance(taskService,
				createSensorConfiguration());
	}

	/**
	 * Creates a plugin instance for testing which is coupled to a
	 * {@link SensorInstanceProvider}'s configuration, i.e. reuses the
	 * {@link SensorTaskRepository} instance from the instance provider to
	 * interact with tasks.
	 * 
	 * @param sensorInstanceProvider
	 *            a sensorProvider which provides access to a
	 *            {@link SensorTaskRepository}
	 * @return an instance of {@link SensorPlugin} for testing purposes.
	 * @throws InternalServiceException
	 *             if instance creation fails
	 */
	public static SensorPlugin createInstance(
			SensorInstanceProvider sensorInstanceProvider)
			throws InternalServiceException {
		SensorTaskRepository taskRepository = sensorInstanceProvider
				.getSensorTaskRepository();
		SensorTaskService taskService = new SensorTaskService(taskRepository);
		return new SimpleSensorPluginTestInstance(taskService,
				createSensorConfiguration());
	}
	...
	private static SensorConfiguration createSensorConfiguration() {
		...
		SensorDescription sensorDescription = new SensorDescription(
				PROCEDURE_DESCRIPTION_FORMAT, "http://my.link.com/");
	...
	public static SensorPlugin createInstance(
			...
			throws InternalServiceException {
		return new SimpleSensorPluginTestInstance(sensorTaskService,
				sensorConfiguration);
	}

	private SimpleSensorPluginTestInstance(SensorTaskService sensorTaskService,
	...
	public Availability getResultAccessibility() {
		LanguageStringType testMessage = LanguageStringType.Factory
				.newInstance();
