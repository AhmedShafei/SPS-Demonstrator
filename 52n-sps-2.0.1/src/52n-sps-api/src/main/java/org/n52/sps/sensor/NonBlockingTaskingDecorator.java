...
/**
 * A {@link SensorPlugin} which executes all tasking requests asynchronously.
 * Wrap a {@link SensorPlugin} instance with a
 * {@link NonBlockingTaskingDecorator} to avoid framework freezing if the
 * wrapped instance gets stuck when executing tasking requests.
 */
public class NonBlockingTaskingDecorator extends SensorPlugin {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(NonBlockingTaskingDecorator.class);

	private static final Executor taskingExecutor = Executors
	...
	public static SensorPlugin enableNonBlockingTasking(
	...
	@Override
	public SensorTask submit(final SubmitTaskingRequest submit,
			final OwsExceptionReport owsExceptionReport) throws OwsException {
		try {
			FutureTask<SensorTask> createSensorTask = new FutureTask<SensorTask>(
					new Callable<SensorTask>() {
						public SensorTask call() throws Exception {
							return sensorPlugin.submit(submit,
									owsExceptionReport);
						}
					});
			taskingExecutor.execute(createSensorTask);
			return createSensorTask.get(NON_BLOCK_TIMEOUT,
					TimeUnit.MILLISECONDS);
		} catch (ExecutionException e) {
			LOGGER.warn("No SensorTask could be created for submit request {}",
					submit, e);
			NoApplicableCodeException ex = new NoApplicableCodeException(
			...
			LOGGER.error(
					"No SensorTask could be created for submit request {}",
					submit, e);
			NoApplicableCodeException ex = new NoApplicableCodeException(
					OwsException.INTERNAL_SERVER_ERROR);
			...
			LOGGER.error("Unacceptable delay creating sensor task (> {} ms)",
					NON_BLOCK_TIMEOUT, e);
			NoApplicableCodeException ex = new NoApplicableCodeException(
					OwsException.INTERNAL_SERVER_ERROR);
