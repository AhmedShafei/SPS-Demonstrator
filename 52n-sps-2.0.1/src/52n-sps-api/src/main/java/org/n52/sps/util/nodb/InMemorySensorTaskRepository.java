...
public class InMemorySensorTaskRepository implements SensorTaskRepository {
	...
	public SensorTask createNewSensorTask(String procedure) {
		// A.S: make separator configurable? => use
		// IdWithPrefixGenerator.DEFAULT_ID_SEPARATOR and change it to ":"
		String taskId = taskIdGenerator.generatePrefixedId(procedure,
				IdWithPrefixGenerator.DEFAULT_ID_SEPARATOR);
	...
	public void saveOrUpdateSensorTask(SensorTask sensorTask) {
		SensorTaskKey key = new SensorTaskKey(sensorTask.getProcedure(),
				sensorTask.getTaskId());
		sensorTasks.put(key, sensorTask);
	}

	public void removeSensorTask(SensorTask sensorTask) {
		sensorTasks.remove(new SensorTaskKey(sensorTask.getProcedure(),
				sensorTask.getTaskId()));
	}
	...
	public SensorTask getSensorTask(String procedure, String taskId)
