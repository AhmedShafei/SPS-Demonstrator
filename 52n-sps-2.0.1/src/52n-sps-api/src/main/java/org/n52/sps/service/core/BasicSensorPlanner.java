 ...
 * Describes functions required by Conformatance Class:
...
public interface BasicSensorPlanner extends CapabilitiesInterceptor {
	...
	final static String DESCRIBE_TASK_RESULT_ACCESS = "DescribeTaskResultAccess";

	final static String DESCRIBE_SENSOR_RESULT_ACCESS = "DescribeSensorResultAccess";
	...
	/**
	 * allows a client to retrieve information, which enables access to the data
	 * produced by the asset. The server response may contain references to any
	 * kind of data accessing OGC Web services such as SOS, WMS, WCS or WFS.
	 * 
	 ...
	 */
	public XmlObject describeResultAccess(
			DescribeResultAccessDocument describeResultAccess)
			throws OwsException, OwsExceptionReport;

	/**
	 * Allows a client to request the information that is needed in order to
	 * prepare a tasking request targeted at the assets that are supported by
	 * the SPS and that are selected by the client. The server will return
	 * information about all parameters that have to be set by the client in
	 ...
	 * @throws OwsExceptionReport
	 */
	public XmlObject describeTasking(DescribeTaskingDocument describeTasking)
			throws OwsException, OwsExceptionReport;

	/**
	 * Allows a client to request and receive service metadata documents that
	 * describe the capabilities of the specific server implementation. This
	 * operation also supports negotiation of the specification version being
	 * used for client-server interactions.
	 * 
	...
	/**
	 * Allows a client to receive information about the current status of the
	 * requested task.
	 * 
	 * @param getStatus
	 *            the request
	 ...
	 * @throws OwsExceptionReport
	 */
	public XmlObject getStatus(GetStatusDocument getStatus)
			throws OwsException, OwsExceptionReport;
	 ...
	 * @throws OwsExceptionReport
	 */
	public XmlObject getTask(GetTaskDocument getTask) throws OwsException,
			OwsExceptionReport;

	/**
	 * Submits a task. Depending on the facaded asset, it may perform a simple
	 * modification of the asset or start a complex mission.
	 * 
	 * @param submit
	 *            the request
	...
	/**
	 * updates sensorTask status report.
	 * 
	 * @param statusReport
	 *            the Status report
	 */
